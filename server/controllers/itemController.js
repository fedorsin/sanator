...
class ItemController {
    async create(req, res, next){
        try {
            let {name, price, typeId, cityId, info, _info, site, address, phone, latitude, longitude} = req.body
            const {img} = req.files
            let fileName = uuid.v4() + '.jpg'
            img.mv(path.resolve(__dirname, '..', 'static', fileName))
            const item = await Item.create({name, price, typeId, cityId, _info, site, address, phone, latitude, longitude, img: fileName})
    ...
    async update(req, res, next){
        try {
            const {id} = req.params
            const {name, price, typeId, cityId, info, _info, site, address, phone, latitude, longitude} = req.body
